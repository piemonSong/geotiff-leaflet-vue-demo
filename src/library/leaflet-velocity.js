...
L.VelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  getIdentify:function(event){
    let self = this;
    let pos = this._map.containerPointToLatLng(L.point(event.containerPoint.x, event.containerPoint.y));
    var gridValue = this._windy.interpolatePoint(pos.lng, pos.lat);
    return gridValue? {Direction:  self.vectorToDegrees(gridValue[0], gridValue[1], this.options.angleConvention).toFixed(2) + "°"  ,
      Speed: self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit).toFixed(2) + this.options.speedUnit}:{}
  },
  vectorToSpeed: function vectorToSpeed(uMs, vMs, unit) {
    var velocityAbs = Math.sqrt(Math.pow(uMs, 2) + Math.pow(vMs, 2));
    // Default is m/s
